...
namespace Obilet.Web.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var userCacheResult = _redisCacheService.Get<UserModel>(cacheKeyUser);

            GetBusLocationResponseModel busLocationResponse = new();
            #endregion

            #region In memory User 
            if (userCacheResult == null)
            {
            ...
            userCacheResult = _redisCacheService.Get<UserModel>(cacheKeyUser);
            busLocationResponse = _obiletService.GetBusLocation(new GetBusLocationRequestModel { Language = "tr-TR", DeviceSession = new DeviceSession { DeviceId = userCacheResult.DeviceId, SessionId = userCacheResult.SessionId }, Date = DateTimeOffset.Now });
            ...
            if (busLocationResponse.Status.ToLower() == "success")
            {
                ...
                return View(new TripModel { BusLocations = busLocationResponse, LastSearchModel = new LastSearchModel { LastOriginId = userCacheResult.LastSearchModel.LastOriginId, LastDestinationId = userCacheResult.LastSearchModel.LastDestinationId, LastDepartureDate = userCacheResult.LastSearchModel.LastDepartureDate } });
            }
            else
            {
        ...
        public IActionResult GetBusJourney(TripModel data)
        {
            ...
            #region Get BusLocations in Redis Cache
            var cacheKeyBusLocations = string.Format(BusLocations);
